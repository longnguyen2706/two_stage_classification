...
def resnet152_model(img_rows, img_cols,  channels, weights_path, num_classes=None):
    """
    ...
    if K.image_dim_ordering() == 'tf':
      bn_axis = 3
      img_input = Input(shape=(img_rows, img_cols, channels), name='data')
    else:
      bn_axis = 1
    ...
    model = Model(img_input, x) # remove top layers that refer to 1000 imagenet class

    '''
    Comment out the original code that returns complete model 
    ...
    # Learning rate is changed to 0.001 -> comment out this compile
